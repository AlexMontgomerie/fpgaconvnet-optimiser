...
# from fpgaconvnet.models.partition.metrics import get_partition_latency, get_partition_throughput
from fpgaconvnet.models.layers.metrics import get_layer_resources_all
...
def create_report(self, output_path):
    ...
    latency = get_network_latency(self.net, self.platform)
    throughput = get_network_throughput(self.net, self.platform)
    ...
    report = {
        ...
        "network" : {
            ...
            "performance" : {
                ...
                "cycles" : self.net.get_cycles(self.multi_fpga),
            ...
            "avg_resource_usage_percentage" : {
                "LUT" : np.mean([ self.get_partition_resource(partition)["LUT"] for partition in self.net.partitions ]) / self.platform.get_resource("LUT") * 100,
                "FF" : np.mean([ self.get_partition_resource(partition)["FF"] for partition in self.net.partitions ]) / self.platform.get_resource("FF") * 100,
                "BRAM" : np.mean([ self.get_partition_resource(partition)["BRAM"] for partition in self.net.partitions ]) / self.platform.get_resource("BRAM") * 100,
                "DSP" : np.mean([ self.get_partition_resource(partition)["DSP"] for partition in self.net.partitions ]) / self.platform.get_resource("DSP") * 100
            },
            "sum_resource_usage" : {
    ...
    if self.platform.get_resource("URAM") > 0:
        ...
        report["network"]["avg_resource_usage_percentage"]["URAM"] = np.mean([ self.get_partition_resource(partition)["URAM"] for partition in self.net.partitions ]) / self.platform.get_resource("URAM") * 100
    ...
    for i in range(len(self.net.partitions)):
        ...
        report["partitions"][i] = {
            ...
            # "latency" : latency,
            "cycles" : self.net.partitions[i].get_cycles(),
            ...
            "resource_usage_percentage" : {
                "LUT" : (resource_usage["LUT"] / self.platform.get_resource("LUT")) * 100,
                "FF" : (resource_usage["FF"] / self.platform.get_resource("FF")) * 100,
                "BRAM" : (resource_usage["BRAM"] / self.platform.get_resource("BRAM")) * 100,
                "DSP" : (resource_usage["DSP"] / self.platform.get_resource("DSP") * 100)
            },
            # "bandwidth" : {
        ...
        if self.platform.get_resource("URAM") > 0:
            ...
            report["partitions"][i]["resource_usage_percentage"]["URAM"] = (resource_usage["URAM"] / self.platform.get_resource("URAM")) * 100

        # add information for each layer of the partition
        report["partitions"][i]["layers"] = {}
        max_cycles = max([self.net.partitions[i].graph.nodes[node]['hw'].cycles() for node in self.net.partitions[i].graph.nodes()])
        for node in ordered_node_list(self.net.partitions[i].graph):
            hw = self.net.partitions[i].graph.nodes[node]['hw']
            resource_usage = get_layer_resources_all(hw, self.platform)
            report["partitions"][i]["layers"][node] = {
                "type" : str(self.net.partitions[i].graph.nodes[node]['type']),
                "interval" : hw.cycles(), #TODO
                "cycle" : hw.cycles(),
                "resource_usage" : {
                    "LUT" : resource_usage["LUT"],
                    "FF" : resource_usage["FF"],
            ...
            if "URAM" in resource_usage.keys() and "URAM" in self.platform.resources:
                ...
                report["partitions"][i]["layers"][node]["resource_usage_percentage"]["URAM"] = (resource_usage["URAM"] / self.platform.get_resource("URAM")) * 100
            ...
            #     bw_weight = bw_weight * hw.cycles() / max_cycles
