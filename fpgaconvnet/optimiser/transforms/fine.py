...
def apply_more_fine(partition, reject_list=[], skip_second_slowest_node=False, conservative_gain = 1.2):
    # feasible layers
...
def apply_more_fine_aggressive(partition, reject_list=[], skip_second_slowest_node=False):
    # feasible layers
    feasible_layers = get_all_layers(partition.graph, LAYER_TYPE.Convolution)
    feasible_layers = [ layer for layer in feasible_layers if layer not in reject_list ]
