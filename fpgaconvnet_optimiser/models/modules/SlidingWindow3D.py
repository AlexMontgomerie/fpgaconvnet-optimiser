...
from fpgaconvnet_optimiser.models.modules import Module3D
...
class SlidingWindow3D(Module3D):
    ...
    def __post_init__(self):
        ...
        # format stride as a 3 element list
        if isinstance(self.stride, int):
            self.stride = [self.stride, self.stride, self.stride]
        elif isinstance(self.stride, list):
            assert len(self.stride) == 3, "Must specify three stride dimensions"
        else:
            raise TypeError
    ...
    def depth_out(self):
        return int((self.depth_in()-self.kernel_size[0]+self.pad_front+self.pad_back)/self.stride[0]+1)

    def rows_out(self):
        return int((self.rows_in()-self.kernel_size[1]+self.pad_top+self.pad_bottom)/self.stride[1]+1)

    def cols_out(self):
        return int((self.cols_in()-self.kernel_size[2]+self.pad_left+self.pad_right)/self.stride[2]+1)

    def rate_in(self):
        return 1.0 # TODO: maybe need to reduce for padding effect

    def rate_out(self):
        return (self.depth_out()*self.rows_out()*self.cols_out())/float(self.depth*self.rows*self.cols)
    ...
    def functional_model(self, data):
        ...
        assert data.shape[1] == self.depth   , "ERROR: invalid depth dimension"
        assert data.shape[2] == self.rows    , "ERROR: invalid row dimension"
        assert data.shape[3] == self.cols    , "ERROR: invalid column dimension"
        ...
        for index,_ in np.ndenumerate(data_padded):
            if  (index[1] < self.pad_back):
                data_padded[index] = 0
            elif(index[2] < self.pad_bottom):
                data_padded[index] = 0
            elif(index[3] < self.pad_left):
                data_padded[index] = 0
            elif(index[1] > self.depth - 1 + self.pad_back):
                data_padded[index] = 0
            elif(index[2] > self.rows - 1 + self.pad_bottom):
                data_padded[index] = 0
            elif(index[3] > self.cols - 1 + self.pad_left):
                data_padded[index] = 0
            else:
        ...
        for index,_ in np.ndenumerate(out):
            out[index] = data_padded[
                index[0],
                index[1]*self.stride[0]+index[5],
                index[2]*self.stride[1]+index[6],
                index[3]*self.stride[2]+index[7],
                index[4]]
