...
class Module3D:
    ...
    def latency(self):
        ...
        int
            ...
            the two.
        """
        latency_in  = int((self.depth_in()*self.rows_in()*self.cols_in()*self.channels_in() )/self.rate_in() )
        latency_out = int((self.depth_out()*self.rows_out()*self.cols_out()*self.channels_out())/self.rate_out())
