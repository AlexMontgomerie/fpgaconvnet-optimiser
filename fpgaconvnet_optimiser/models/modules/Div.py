...
Calculates the input A divided by b (single divisor value).
For use in softmax layer.
...
class Div(Module):
    def __init__(
        ...
        #TODO resource coefficients file for Div module
    ...
    def functional_model(self, data, divisor):
        # check input dimensionality
        assert data.shape[0] == self.rows    , "ERROR: invalid row dimension"
        assert data.shape[1] == self.cols    , "ERROR: invalid column dimension"
        assert data.shape[2] == self.channels, "ERROR: invalid channel dimension"

        out = np.true_divide(data, divisor)
        return out
