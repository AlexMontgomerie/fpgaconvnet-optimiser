...
class InnerProductLayer(Layer):
    def __init__(
        ...
        # init modules
        self.modules = {
            "fork"           : Fork( [self.channels,self.rows,self.cols],1,coarse_out,batch_size,data_width),
            "conv"           : Conv( [self.channels*self.rows*self.cols,1,1],filters,1,1,1,data_width,acc_width,weight_width),
            "accum"          : Accum([self.channels*self.rows*self.cols,1,1],filters,1,data_width),
            "glue"           : Glue( [self.channels*self.rows*self.cols,1,1],filters,coarse_in,coarse_out,data_width,acc_width)
        }
        self.update()
